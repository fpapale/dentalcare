...
| **CF non è chiave primaria** | ✅ | `patients.id` è UUID interno — conforme a §5.1 e all'errore §28.6 |
| **CF opzionale** | ❌→✅ | **Correzione 17/07/2026:** questa riga diceva *"il CF è opzionale e validato"*. **Era falso.** `FiscalCodeValidator` lo rendeva **obbligatorio** salvo dichiarare il paziente straniero, e l'errore §28.6 era quindi solo mezzo evitato: il CF non era la chiave, ma era comunque un requisito d'ingresso. Conseguenza pratica: l'assistente vocale, a cui il prompt **vieta** di chiederlo (minimizzazione), non poteva creare nessun paziente — ogni prenotazione telefonica di un paziente nuovo falliva con 400. Risolto in `09dc68b`: opzionale alla creazione, validato quando presente. Resta da fare il controllo all'emissione della fattura (proposta #26) |
