...
--
-- Prerequisiti:
--   - 01_schema_applicative.sql applicato (enum e funzioni globali disponibili)
...
-- =============================================================================
-- FUNZIONE DI SERVIZIO (ridefinita nel tenant schema per sicurezza)
-- =============================================================================
...
-- =============================================================================
-- 2. PATIENTS
-- Anagrafica pazienti della clinica.
-- =============================================================================
...
-- =============================================================================
-- 3. PROVIDERS
-- Operatori della clinica (dentisti, igienisti, chirurghi, assistenti, admin).
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS providers (
    ...
    email                citext,
    license_number       text,
    -- Colore agenda per identificazione visiva
    color_hex            char(7)       NOT NULL DEFAULT '#4F46E5',
    active               boolean       NOT NULL DEFAULT true,
    -- Dati fatturazione (per provider a P.IVA)
    vat_number           text,
...
-- =============================================================================
-- 4. SERVICE_CATALOG
-- Listino prestazioni della clinica.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS service_catalog (
    ...
    -- Applicabilita' per posizione dentale (notazione FDI, seconda cifra)
...
-- =============================================================================
-- 5. TREATMENT_PLANS
-- Piano di cura clinico per paziente.
-- =============================================================================
...
-- Singole voci (prestazioni) di un piano di cura.
-- tooth_fdi: notazione FDI es. '16', '36', '11'
-- =============================================================================
...
-- =============================================================================
-- 7. APPOINTMENTS
-- Appuntamenti in agenda (per paziente, provider, poltrona).
-- =============================================================================
...
-- =============================================================================
-- 8. ESTIMATES
-- Preventivi economici per il paziente.
-- I totali sono ricalcolati automaticamente dal trigger su estimate_lines.
-- =============================================================================
...
-- =============================================================================
-- 9. ESTIMATE_LINES
-- Righe del preventivo. Il totale del preventivo e' ricalcolato via trigger.
-- discount_pct: sconto percentuale sulla riga (0-100)
...
-- =============================================================================
-- 10. INVOICES
-- Fatture, ricevute, note di credito emesse dalla clinica o dal provider.
-- =============================================================================
...
-- =============================================================================
-- 11. INVOICE_LINES
-- Righe della fattura. Il totale fattura e' ricalcolato via trigger.
-- vat_rate: aliquota IVA percentuale (0, 4, 10, 22)
-- =============================================================================
...
-- Scheda anamnestica strutturata del paziente.
-- is_current=true identifica la versione attiva (una sola per paziente/clinica).
-- =============================================================================
...
-- Solo una scheda corrente per paziente/clinica
...
-- Selezioni strutturate di voci amnestiche per un paziente.
-- Fa riferimento alle voci globali in dentalcare.anamnesis_items.
-- =============================================================================
...
-- =============================================================================
-- 14. ODONTOGRAM_TEETH
-- Stato corrente di ogni dente per paziente (una riga per dente).
-- =============================================================================
...
-- =============================================================================
-- 15. TOOTH_CONDITIONS
-- Storico delle condizioni dentali (ogni rilevamento crea una nuova riga).
-- =============================================================================
...
-- Note cliniche di seduta (diario clinico).
-- =============================================================================
...
-- Documenti allegati al paziente (RX, foto, consensi, referti).
-- =============================================================================
...
-- =============================================================================
-- 18. SUPPLIERS
-- Fornitori di prodotti e materiali per la clinica.
-- =============================================================================
...
-- Categorie merceologiche per i prodotti di magazzino.
-- =============================================================================
...
-- =============================================================================
-- 20. PRODUCTS
-- Prodotti e materiali in magazzino.
-- =============================================================================
...
-- =============================================================================
-- 21. STOCK_MOVEMENTS
-- Movimenti di magazzino (carico, scarico, rientro, rettifica).
-- quantity e' sempre positivo; il segno e' determinato da movement_type.
-- =============================================================================
...
-- Definisce le sotto-prestazioni automaticamente suggerite per una prestazione
-- principale (es. estrazione → rimozione punti).
-- =============================================================================
...
-- Per ogni condizione odontogramma, definisce le prestazioni suggerite
-- quando si genera un piano di cura automatico.
-- =============================================================================
...
-- =============================================================================
-- 24. PATIENT_RECALLS
-- Richiami programmati per i pazienti (igiene periodica, follow-up, ecc.).
-- =============================================================================
...
-- =============================================================================
-- 25. RECALL_CONTACTS
-- Storico dei tentativi di contatto per un richiamo.
-- =============================================================================
...
-- =============================================================================
-- 26. AI_CONVERSATIONS
-- Conversazioni con l'assistente AI per analisi clinica e supporto decisionale.
-- messages: array JSONB di messaggi {role: 'user'|'assistant', content: '...', ts: ...}
-- =============================================================================
...
-- Ricalcola i totali di un preventivo dalle sue righe
...
BEGIN
    ...
    SET
        ...
        discount_total = COALESCE((
            SELECT SUM(unit_price * quantity * discount_pct / 100)
            FROM estimate_lines
            WHERE estimate_id = v_estimate_id
        ), 0),
        total          = COALESCE((
...
-- Ricalcola i totali di una fattura dalle sue righe
...
BEGIN
    ...
    SET
        ...
        vat_total  = COALESCE((
            SELECT SUM(unit_price * quantity * vat_rate / 100)
            FROM invoice_lines
            WHERE invoice_id = v_invoice_id
        ), 0),
        total      = COALESCE((
...
-- Calcola la priorita' di un richiamo in base alla scadenza
...
-- Aggiorna lo stato del richiamo in base all'esito del contatto
...
-- Vista: riepilogo per paziente (dashboard paziente)
...
-- Vista: cartella clinica paziente con alert anamnestici
...
-- Vista: giacenza corrente per prodotto
...
-- Vista: agenda giornaliera arricchita con alert paziente
...
SELECT
    ...
    EXISTS (
        ...
        WHERE pa2.patient_id  = p.id
          AND pa2.clinic_id   = a.clinic_id
          AND pa2.is_current  = true
          AND (pa2.allergy_penicillin OR pa2.allergy_latex
               OR pa2.allergy_anesthetic OR pa2.allergy_aspirin
               OR pa2.other_allergies IS NOT NULL)
...
-- Vista: riepilogo preventivi per paziente
...
-- Vista: KPI clinica (dashboard di riepilogo)
...
provider_agg AS (
    SELECT clinic_id,
           ...
           COUNT(*) FILTER (WHERE active = true AND role IN ('dentist','hygienist','orthodontist','surgeon'))
                                                                                        AS clinical_providers
    FROM providers
    GROUP BY clinic_id
),
plan_agg AS (
