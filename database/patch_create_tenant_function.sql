-- Patch: aggiorna SOLO la funzione dentalcare.create_tenant.
--
-- Estratta da database/install.sql, che resta la fonte di verita': se il DDL
-- del tenant cambia, si rigenera questo file da li' invece di editarlo a mano.
--
-- Serve quando lo schema tenant e' evoluto ma non si vuole toccare il database:
-- CREATE OR REPLACE sostituisce la funzione e basta. Nessuna tabella letta o
-- scritta, nessun dato modificato, nessuno schema tenant esistente alterato.
-- I tenant gia' creati NON cambiano: restano allineati da patchSchema all'avvio
-- del backend. Cambiano solo i tenant creati DOPO questa patch.
--
-- Uso:
--   psql -U postgres -h <host-db> -d dentalcare_prod -f database/patch_create_tenant_function.sql
--
-- Allineamento a questa versione (verificato per confronto con il tenant demo
-- di sviluppo t_9d754153): 41 oggetti, nessuna colonna mancante o in eccesso.
-- Include work_start_time/work_end_time/slot_minutes/working_days su clinics (#31)
-- e service_categories.allowed_roles (#36).

\echo '=== firma della funzione PRIMA ==='
SELECT p.oid::regprocedure AS funzione,
       md5(pg_get_functiondef(p.oid)) AS impronta
FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'dentalcare' AND p.proname = 'create_tenant';

\echo '=== sostituzione ==='
CREATE OR REPLACE FUNCTION dentalcare.create_tenant(p_tenant_id uuid, p_clinic_id uuid, p_schema text, p_studio_name text, p_email text, p_phone text, p_plan text, p_vat text, p_address text, p_city text, p_province text, p_admin_first text, p_admin_last text, p_admin_email text, p_admin_pw_hash text) RETURNS uuid
    LANGUAGE plpgsql
    AS $_$
...
\echo '=== firma della funzione DOPO (impronta deve essere cambiata) ==='
SELECT p.oid::regprocedure AS funzione,
       md5(pg_get_functiondef(p.oid)) AS impronta
FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'dentalcare' AND p.proname = 'create_tenant';

\echo '=== verifica: la nuova funzione dichiara le colonne orari? ==='
SELECT count(*) FILTER (WHERE pg_get_functiondef(p.oid) LIKE '%work_start_time%') AS ha_orari,
       count(*) FILTER (WHERE pg_get_functiondef(p.oid) LIKE '%allowed_roles%')   AS ha_ruoli
FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'dentalcare' AND p.proname = 'create_tenant';
