<!-- converted from DentalCare_Guida_Digitalizzazione_Cartella_Clinica_Dentale.docx -->

DentalCare Pro — Guida approfondita alla digitalizzazione della cartella clinica del paziente dentale
Documento operativo e architetturale
16 luglio 2026
# Premessa
...
identificare con certezza paziente, professionista, struttura e episodio di cura;
raccogliere anamnesi, esame obiettivo, odontogramma, diagnosi, piano di trattamento, procedure e follow-up in forma strutturata;
collegare fotografie, radiografie, DICOM, referti, consensi e prescrizioni al corretto episodio clinico;
mantenere la storia completa delle modifiche senza sovrascrivere il passato;
applicare firme, validazioni e conservazione dove necessario;
consentire accessi per ruolo, contesto e relazione di cura;
produrre documenti leggibili dal paziente e dati interoperabili con altri sistemi;
integrare moduli AI senza confondere l'output algoritmico con la decisione clinica;
dimostrare a un controllo chi ha visto o modificato cosa, quando e perché;
migrare dal cartaceo senza perdita di informazione o interruzione dell'attività.
...
addendum;
rettifica motivata;
annullamento logico con conservazione dell'originale;
nuova versione collegata alla precedente.
...
attribuzione applicativa: autenticazione forte, utente individuale, timestamp, audit;
finalizzazione clinica: approvazione esplicita della nota e blocco delle modifiche;
firma elettronica: per consensi, referti, certificati e documenti previsti dalla policy;
firma qualificata/digitale e marca temporale: quando richiesta dal flusso o scelta per maggior valore probatorio;
PAdES: per documenti PDF destinati a specifici flussi, incluso il processo FSE 2.0 descritto nelle specifiche di accreditamento [R6].
...
Row Level Security;
namespace/bucket policy;
cache segregate;
code review dedicata;
test automatici cross-tenant;
chiavi per tenant dove giustificato;
amministrazione separata;
monitoraggio di query anomale.
...
generare il documento clinico previsto;
costruire CDA2 secondo la specifica applicabile;
applicare terminologie e schematron pubblicati;
generare il PDF con CDA integrato secondo il flusso richiesto;
apporre firma PAdES;
autenticarsi con certificati e JWT;
invocare validazione;
gestire pubblicazione, sostituzione, annullamento e metadati;
memorizzare esito, identificatori e ricevute;
gestire specificità regionali e accreditamento.
...
presa in carico e inventario;
preparazione fascicolo;
assegnazione barcode/ID;
scansione;
controllo leggibilità e completezza;
OCR per indicizzazione, non come fonte clinica automatica;
classificazione;
associazione al paziente;
doppio controllo per campioni o categorie critiche;
hash e import;
verbale di esito;
gestione del cartaceo secondo policy.
...
scansionare tutto e chiamarlo cartella digitale;
usare account condivisi;
consentire alla segreteria l'accesso indiscriminato;
sovrascrivere note finalizzate;
salvare immagini con nome paziente nel filesystem;
usare il codice fiscale come chiave primaria;
confondere backup e conservazione;
inviare dati sanitari a LLM pubblici;
generare note precompilate non veritiere;
migrare senza riconciliazione;
cancellare il cartaceo senza policy;
costruire il database copiando un unico standard di scambio;
aggiungere il FSE come esportazione manuale non tracciata;
addestrare l'AI con correzioni non validate;
ignorare la provenienza dei dati;
non testare il ripristino;
non registrare download e stampe;
permettere agli amministratori tecnici di leggere tutto;
fare go-live senza downtime procedure;
non coinvolgere odontoiatri nei test.
